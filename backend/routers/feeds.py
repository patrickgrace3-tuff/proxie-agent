...
Feeds are GLOBAL — created by admins, visible to all active users.
Jobs are scored per-user against their own rules.
...
def require_admin(user: dict = Depends(get_current_user)):
    ...
    with db() as cur:
        ...
        row = cur.fetchone()
    if not row or row_to_dict(row).get("role") != "admin":
...
def parse_xml_feed(content: str, field_map: dict = None, feed_id: int = 0) -> list[dict]:
    ...
    items = (
        root.findall('.//job') or root.findall('.//Job') or
        root.findall('.//item') or root.findall('.//position') or
        root.findall('.//listing') or root.findall('.//vacancy') or list(root)
    )
...
def parse_json_feed(content: str, field_map: dict = None, feed_id: int = 0, default_carrier: str = "", default_phone: str = "") -> list[dict]:
    ...
    for row in data:
        ...
        carrier = _get(row, fm.get('carrier_name', ''),
        ...
        title = _get(row, fm.get('job_title', ''),
...
async def fetch_and_parse(feed: dict) -> list[dict]:
    ...
    if is_url:
        async with httpx.AsyncClient(timeout=30) as c:
            resp = await c.get(source, headers={'User-Agent': 'DriverAgent/1.0'})
...
def score_job(job: dict, rules: AgentRules) -> int:
    ...
    carrier = {
        'name': job.get('carrier_name', ''), 'location': location,
        'cpm': cpm, 'weekly_pay_estimate': weekly,
        'home_time': job.get('home_time', ''), 'freight_types': [],
        'operating_states': operating_states, 'no_touch': False,
        'drop_and_hook': False, 'forced_dispatch': False,
        'lease_only': False, 'sign_on_bonus': False,
        'health_insurance': False, 'retirement_plan': False,
    }
...
async def create_feed(feed: FeedCreate, user: dict = Depends(require_admin)):
    ...
    with db() as cur:
        cur.execute("""
            ...
            VALUES (%s, %s, %s, %s, %s, %s)
            RETURNING id
        """, (admin_id, feed.name, feed.feed_type, feed.source,
...
def get_feed_jobs(
    ...
    with db() as cur:
        ...
        rows = cur.fetchall()
        cur.execute(
            f"SELECT COUNT(*) as cnt FROM feed_jobs fj LEFT JOIN carrier_feeds cf ON fj.feed_id=cf.id WHERE {where_clause}",
            params
        )
    ...
    return {
        ...
        "total_pages": total_pages, "rules_active": rules_active,
